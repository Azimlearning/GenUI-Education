...
| **P4** | Polish + demo script | ✅ **Done (2026-07-01).** Per-subject accent theming (Bio green, Chem sand, Physics blue) keyed off the block subject; "How Synapse is different" explainer on the landing page; footnote/chips refreshed. Demo beat sheet lives in `refdocs/VIDEO-SCRIPT.md`. |
| **hackAstone** | Teacher view, grounded pedagogy, go-to-market, evaluation, video, description | 🟡 **In progress (2026-07-01).** Teacher dashboard live (`/teacher` + `GET /api/teacher`); misconception KB expanded with real sourced citations (Odom & Barrow, Taber, Clement/Halloun-Hestenes, Driver, Shipstone); deploy config built (Dockerfile + vercel.json), not deployed. Written deliverables (go-to-market, evaluation, video script, project description, README) landing next. |
