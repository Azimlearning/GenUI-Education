...
3. Interactivity integrity. Every plan variable must have a control, and every control must actually change the computation or rendering. A dead slider, a control wired to nothing, or a reset that does not restore defaults is a blocker. If `interaction_mode` is `drag_drop`, trace the pointer interaction: a visibly draggable named object must drop onto the planned target, change the true scientific trial state, emit an `axiom_event`, and lead to a measurable outcome. A button substitute or decorative drag is a blocker.
...
5. Pedagogical clarity. Quantities labeled with units, a visible one-line 'Try this' hint, a reset button, readouts that make the consequence of each control observable. The plan's study_note text must appear inside the artifact as a visible card (not omitted, not buried), since the artifact now fills most of the screen and is the learner's only note. If plan.steps is non-empty, a numbered steps strip must be visibly rendered. Missing labels, hint, note card, or steps strip are major, not blockers, unless the artifact is unusable without them.
